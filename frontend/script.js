...
async function kayitOl() {
    ...
    try {
        ...
        if (res.ok) { alert("Kayıt Başarılı! Şimdi Giriş Yapabilirsiniz."); }
...
async function profilSil() {
    ...
    try {
        ...
        if (res.ok) { alert("Hesabınız silindi."); location.reload(); }
        else { let msg = "Silme başarısız!"; try { const d = await res.json(); msg = d.error || msg; } catch (_) {} alert(msg); }
    } catch (err) { alert("Bağlantı hatası: " + err.message); }
}
...
async function ilanVer() {
    ...
    try {
        ...
        if (res.ok) { alert("İlan Yayınlandı!"); document.getElementById('ad-title').value = ""; yetenekleriGetir(); }
        else { let msg = "İlan yayınlanamadı!"; try { const d = await res.json(); msg = d.error || msg; } catch (_) {} alert(msg); }
    } catch (err) { alert("Bağlantı hatası: " + err.message); }
}

// 6) İLANLARI LİSTELE
// ✅ DÜZELTİLDİ: addEventListener KALDIRILDI — index.html'deki DOMContentLoaded yeterli
async function yetenekleriGetir() {
    try {
        ...
        listDiv.innerHTML = users.map(user => {
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center; background:#f8fafc; padding:8px 12px; border-radius:10px; margin:6px 0;">
                        ...
                        ${isMe ? `
                            <div style="display:flex; gap:6px;">
                                <button class="btn-ilan-guncelle"
                                    data-id="${ad._id}"
                                    data-title="${encodeURIComponent(ad.title)}"
                                    data-desc="${encodeURIComponent(ad.description || '')}"
                                    style="background:#e0e7ff; color:#4f46e5; border:none; padding:4px 10px; border-radius:8px; cursor:pointer; font-size:12px; font-weight:600;">✏️</button>
                                <button class="btn-ilan-sil"
                                    data-id="${ad._id}"
                                    style="background:#fee2e2; color:#ef4444; border:none; padding:4px 10px; border-radius:8px; cursor:pointer; font-size:12px; font-weight:600;">🗑️</button>
                            </div>
                        ` : ''}
                    </div>`).join('')
            ...
            <div class="user-card" style="border:1px solid #e2e8f0; padding:20px; margin-top:15px; border-radius:16px; background:white; box-shadow:0 4px 6px -1px rgba(0,0,0,0.05);">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                    ...
                    <span style="font-size:13px; background:#dcfce7; color:#22c55e; padding:4px 12px; border-radius:20px; font-weight:bold;">${user.balance} Puan</span>
                </div>
                    ...
                    <div style="display:flex; gap:8px; flex-wrap:wrap;">
                        ${!isMe ? `
                            <a href="mailto:${user.email}?subject=TalentLoop İş Birliği Teklifi"
                               style="background:#22c55e; color:white; padding:8px 14px; text-decoration:none; border-radius:10px; font-size:13px; font-weight:600; display:inline-flex; align-items:center;">✉️ Mesaj Gönder</a>
                            <button class="btn-transfer" data-id="${user._id}"
                                style="background:#4f46e5; color:white; border:none; padding:8px 14px; border-radius:10px; cursor:pointer; font-size:13px; font-weight:600;">💸 1 Puan Gönder</button>
        ...
        // ❌ ESKİ KOD BURADA SILINDI:
        // listDiv.addEventListener('click', handleListClick);
        // Her yetenekleriGetir() çağrısında yeni listener ekleniyordu → adId bozuluyordu → 404

    } catch (err) { alert("Kullanıcılar getirilemedi: " + err.message); }
}
...
async function ilanGuncelle() {
    ...
    try {
        ...
        if (res.ok) { modalKapat('ilanModal'); alert("İlan güncellendi!"); yetenekleriGetir(); }
        else { let msg = "Güncelleme başarısız!"; try { const d = await res.json(); msg = d.error || msg; } catch (_) {} alert(msg); }
    } catch (err) { alert("Bağlantı hatası: " + err.message); }
}
...
async function ilanSil(adId) {
    ...
    try {
        const res = await fetch(`${API}/api/ads/${currentUser._id}/${adId}`, { method: 'DELETE' });
        if (res.ok) { alert("İlan silindi!"); yetenekleriGetir(); }
        else { let msg = "Silme başarısız!"; try { const d = await res.json(); msg = d.error || msg; } catch (_) {} alert(msg); }
    } catch (err) { alert("Bağlantı hatası: " + err.message); }
}
...
async function transfer(toId) {
    ...
    try {
        ...
        const data = await res.json();
        if (res.ok) { alert("Puan Transfer Edildi!"); await bakiyeSorgula(); yetenekleriGetir(); }
        else { alert(data.error || "Transfer başarısız!"); }
    } catch (err) { alert("Bağlantı hatası: " + err.message); }
}
...
async function bakiyeSorgula() {
    ...
    try {
        ...
        } else { let msg = "Bakiye sorgulanamadı!"; try { const d = await res.json(); msg = d.error || msg; } catch (_) {} alert(msg); }
    } catch (err) { alert("Bağlantı hatası: " + err.message); }
}